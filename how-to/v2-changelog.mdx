...
<Accordion title="Updates to Session Objects">
  - Session storage now stores `AgentSession`, `TeamSession` and `WorkflowSession` with new schemas. See full details in the [Session](/concepts/sessions/overview) section.
...
- Added support for custom loggers. See the [documentation](/concepts/custom-logging) for more details.
