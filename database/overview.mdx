...
The agent now persists sessions and includes 3 messages from history in every request.
...
<CardGroup cols={2}>
  ...
  >
    Manage conversation threads with IDs, naming, and caching.
  </Card>
  <Card
    title="Chat History"
  ...
  >
    Control how agents access previous messages.
  </Card>
  <Card
...
For async applications, use the async database classes like `AsyncPostgresDb` or `AsyncSqliteDb`:
```python
from agno.agent import Agent
