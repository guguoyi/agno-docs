...
A session groups related runs into a conversation thread, identified by a `session_id`. Add a database to your agent and sessions are stored automatically with every run's messages, responses, and metadata.
...
<Tip>
You can use separate tables for different agents or environments.
</Tip>
