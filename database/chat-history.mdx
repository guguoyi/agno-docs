...
<Tip>
Start with `num_history_runs=3`. Increase only if your agent needs more context. For long conversations, combine limited history with [session summaries](/database/session-summaries).
</Tip>
...
<Note>
Workflow history passes previous workflow outputs to steps, not conversation messages. See [Workflow Sessions](/database/workflow-sessions) for details.
</Note>
