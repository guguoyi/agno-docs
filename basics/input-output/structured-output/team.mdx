...
Stream the team response while still getting a validated object:
```python
...
for chunk in team.run("Analyze AI market trends", stream=True):
    if chunk.event == TeamRunEvent.run_content:
