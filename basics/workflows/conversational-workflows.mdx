...
Similar to [workflow history for steps](/context/history/workflow/overview), the `WorkflowAgent` has access to the full history of workflow runs for the current session.
...
Explore the different examples in [Conversational Workflows Examples](/basics/workflows/conversational-workflows) section for more details.