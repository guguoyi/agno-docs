...
Similar to [workflow history for steps](/basics/chat_history/workflow), the `WorkflowAgent` has access to the full history of workflow runs for the current session.
...
Explore the different examples in [Conversational Workflows Examples](/examples/basics/workflows/06_workflows_advanced_basics/conversational_workflows/conversational_workflow_with_conditional_step) section for more details.