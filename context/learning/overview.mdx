...
**Learning Machine coordinates multiple learning stores to capture user profiles, observations, session context, entity knowledge, and reusable insights across interactions.**
...
<Tip>
New to Learning Machine? Start with [Getting Started](/context/learning/getting-started).
</Tip>
...
agent = Agent(
    ...
    db=db,
    learning=True,  # Enable user profile learning
)
...
- Extracts user profile information (name, preferred name)
- Recalls profile context in future interactions

For user memory, session context, and other stores, use explicit configuration (see Level 2 below).
